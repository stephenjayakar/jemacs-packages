...
function renderSidebar(editor: Editor): void {
  ...
  render(SESSIONS_NAME, "Debug Sessions", (lines, actions) => {
    ...
    for (const session of st.sessions) {
      actionLine(lines, actions, `${session.state === "stopped" ? "▾" : "▸"} ${session.name}`, { kind: "session", sessionId: session.id })
...
async function startConfigurations(
  ...
  const st = state(editor)
  if (st.sessions.some(session => session.state !== "terminated")) throw new Error("A dap session group is already active")
...
export function install(editor: Editor, ctx: PluginContext = createPluginContext(editor)): void {
  ...
  ctx.minorMode({
    ...
    global: true,
    onEnable(ed) {
      for (const mode of ["dap-mode", "dap-ui-mode", "dap-ui-many-windows-mode", "dap-ui-controls-mode", "dap-tooltip-mode"]) {
  ...
  const evaluateText = async (ed: Editor, expression: string) => {
    const session = state(ed).sessions.find(candidate => candidate.state === "stopped")
    if (!session) { ed.message("No stopped debug session"); return }
  ...
  const eachActive = async (ed: Editor, fn: (session: DapSession) => Promise<void>) => {
    ...
    if (!active.length) { ed.message("No active dap session"); return }
    try { await Promise.all(active.map(fn)) } catch (error) { ed.message(error instanceof Error ? error.message : String(error)) }
  }
  command("dap-continue", async ({ editor }) => eachActive(editor, session => session.continue()), "Continue the selected debug threads.")
  command("dap-pause", async ({ editor }) => eachActive(editor, session => session.pause()), "Pause active debug sessions.")
  command("dap-next", async ({ editor }) => eachActive(editor, session => session.next()), "Step over in active debug sessions.")
  command("dap-step-in", async ({ editor }) => eachActive(editor, session => session.stepIn()), "Step into in active debug sessions.")
  command("dap-step-out", async ({ editor }) => eachActive(editor, session => session.stepOut()), "Step out in active debug sessions.")
  command("dap-debug-restart", async ({ editor }) => eachActive(editor, session => session.restart()), "Restart active debug sessions when supported.")
  command("dap-restart-frame", async ({ editor }) => eachActive(editor, session => session.restartFrame()), "Restart the selected stack frame.")
  command("dap-disconnect", async ({ editor }) => eachActive(editor, session => session.disconnect()), "Terminate or disconnect active debug sessions.")
  command("dap-switch-session", async ({ editor }) => {
    ...
    const currentIndex = state(editor).sessions.indexOf(session)
    state(editor).sessions.splice(currentIndex, 1)
    state(editor).sessions.unshift(session)
    renderSidebar(editor)
  ...
  command("dap-switch-stack-frame", async ({ editor }) => {
    ...
    if (Number.isFinite(frameId)) { await session.selectFrame(frameId); await navigateToFrame(editor, session); await refreshWatches(editor) }
  }, "Select the current stack frame.")
  const moveFrame = async (ed: Editor, delta: number) => {
    const session = state(ed).sessions.find(candidate => candidate.state === "stopped")
    if (!session?.selectedFrame) { ed.message("No stopped debug session"); return }
    const index = session.frames.findIndex(frame => frame.id === session.selectedFrame?.id)
    const frame = session.frames[index + delta]
  ...
  command("dap-up-stack-frame", async ({ editor }) => moveFrame(editor, 1), "Move up one stack frame.")
  command("dap-down-stack-frame", async ({ editor }) => moveFrame(editor, -1), "Move down one stack frame.")
